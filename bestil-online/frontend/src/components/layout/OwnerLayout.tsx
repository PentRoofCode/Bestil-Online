...
import { UtensilsCrossed, ClipboardList, Settings, ChevronLeft, ShoppingCart, LogOut, Store } from "lucide-react";
...
export default function OwnerLayout() {
  ...
  // Auto-redirect single-restaurant owners directly to their order queue
  useEffect(() => {
    if (!restaurantId && restaurants.length === 1) {
      navigate(`/restaurant/${restaurants[0].id}/orders`, { replace: true });
    }
  }, [restaurants, restaurantId, navigate]);
  ...
  const activeRestaurantId = restaurantId ?? (restaurants.length === 1 ? restaurants[0].id : null);
      ...
      <aside className="w-56 shrink-0 border-r border-gray-100 bg-white flex flex-col">
        <div className="flex h-16 shrink-0 items-center gap-2 border-b border-gray-100 px-5">
          ...
          <span className="font-bold text-gray-900 truncate">
            {current?.name ?? "Restaurant Portal"}
          </span>
        </div>
        ...
        <nav className="flex flex-col gap-0.5 p-3 flex-1 overflow-y-auto">
          {activeRestaurantId ? (
            <>
              ...
              {[
                { to: `/restaurant/${activeRestaurantId}/orders`, icon: ClipboardList, label: "Ordrer" },
                { to: `/restaurant/${activeRestaurantId}/menu`, icon: UtensilsCrossed, label: "Menu" },
                { to: `/restaurant/${activeRestaurantId}/settings`, icon: Settings, label: "Indstillinger" },
            ...
            // Multi-restaurant: show all as cards in the sidebar
            restaurants.map((r) => (
              <div key={r.id} className="mb-2">
                <p className="flex items-center gap-1.5 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-gray-400">
                  <Store className="h-3 w-3" />
                  {r.name}
                </p>
                {[
                  { to: `/restaurant/${r.id}/orders`, icon: ClipboardList, label: "Ordrer" },
                  { to: `/restaurant/${r.id}/menu`, icon: UtensilsCrossed, label: "Menu" },
                ].map(({ to, icon: Icon, label }) => (
                  <NavLink
                    key={to}
                    to={to}
